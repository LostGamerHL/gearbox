...
{
	return DefaultDeploy("models/v_shock.mdl", "models/p_shock.mdl", SHOCK_DRAW, "shockrifle");EMIT_SOUND( ENT( pev ), CHAN_ITEM, "weapons/shock_draw.wav", 1, ATTN_NORM );
}

void CShockrifle::Holster(int skiplocal /* = 0 */)
{
	m_pPlayer->m_flNextAttack = gpGlobals->time + UTIL_WeaponTimeBase() + 0.5;
...
void CShockrifle::PrimaryAttack()
{
	...
	{
		m_flNextPrimaryAttack = gpGlobals->time + UTIL_WeaponTimeBase() + 0.2;
	}
